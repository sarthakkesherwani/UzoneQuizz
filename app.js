...
(() => {
  ...
  let _glassDragDidDrag = false; // flag to suppress click after drag
  (function setupLiquidGlassDrag() {
    ...
    function offsetToIndex(px) { return itemWidth > 0 ? Math.round(px / itemWidth) : 0; }

    document.addEventListener('pointerdown', function(e) {
      // Only start from the active dock item (which has the glass behind it)
      const activeItem = e.target.closest('.liquid-dock-item.active');
      if (!activeItem) return;

      dock = activeItem.closest('.liquid-dock');
      if (!dock || dock.dataset.animating === 'true') return;

      indicator = dock.querySelector('.liquid-glass-indicator');
      if (!indicator || indicator.classList.contains('is-hidden')) return;

      const items = dock.querySelectorAll('.liquid-dock-item');
      itemCount = items.length;
      if (itemCount === 0) return;

      itemWidth = items[0].offsetWidth;
      maxOffsetPx = (itemCount - 1) * itemWidth;

      currentSnapIndex = Number(activeItem.dataset.dockIndex || 0);
      startOffsetPx = currentSnapIndex * itemWidth;
      startX = e.clientX;
      startY = e.clientY;
      lastHoverIndex = currentSnapIndex;

      pointerDown = true;
      dragging = false;
      _glassDragDidDrag = false;
    }, { passive: true });

    document.addEventListener('pointermove', function(e) {
      if (!pointerDown || !indicator || !dock) return;
      ...
      const rawOffset = startOffsetPx + dx;
      // Rubber band overscroll
      let offset;
      if (rawOffset < 0) {
        offset = rawOffset * 0.2;
      } else if (rawOffset > maxOffsetPx) {
        offset = maxOffsetPx + (rawOffset - maxOffsetPx) * 0.2;
      } else {
        offset = rawOffset;
      }
      ...
      const hoverIdx = clamp(offsetToIndex(rawOffset), 0, itemCount - 1);
      if (hoverIdx !== lastHoverIndex) {
        lastHoverIndex = hoverIdx;
        haptic();
        dock.querySelectorAll('.liquid-dock-item').forEach((item, i) => {
          item.classList.toggle('pending', i === hoverIdx);
        });
      }
    ...
    function endDrag(e) {
      ...
      const rawOffset = startOffsetPx + dx;
      const snapIndex = clamp(offsetToIndex(rawOffset), 0, itemCount - 1);

      const snapPx = snapIndex * itemWidth;
      const snapPercent = snapIndex * 100;
      dock.style.setProperty('--liquid-offset', `${snapPercent}%`);
      indicator.style.transform = '';

      const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
      if (!reducedMotion) {
        const currentPx = clamp(rawOffset, -itemWidth * 0.2, maxOffsetPx + itemWidth * 0.2);
        indicator.getAnimations().forEach(a => a.cancel());
        indicator.animate([
          { transform: `translate3d(${currentPx}px, 0, 0)` },
          { transform: `translate3d(${snapPx}px, 0, 0)` }
        ], {
          duration: 380,
          easing: 'cubic-bezier(.22, .92, .28, 1)',
          fill: 'forwards'
        ...
        if (snapIndex !== currentSnapIndex) {
          dock.classList.add('is-moving');
          setTimeout(() => dock.classList.remove('is-moving'), 450);
        }
      }
      ...
      if (snapIndex !== currentSnapIndex) {
        haptic();
        const items = dock.querySelectorAll('.liquid-dock-item');
        const targetItem = items[snapIndex];
        if (targetItem) {
          ...
          setTimeout(() => {
            state.page = targetItem.dataset.page;
            state.sidebarOpen = false;
            render();
            window.scrollTo({ top: 0, behavior: 'smooth' });
            refreshPageData(state.page);
    ...
    document.addEventListener('pointercancel', function() {
      ...
      if (dock) {
        dock.querySelectorAll('.liquid-dock-item').forEach(item => item.classList.remove('pending'));
      }
    });
  })();
